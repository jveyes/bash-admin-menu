...
if [ ${#containers[@]} -eq 0 ]; then
    print_box "$(print_colored "$WARNING_COLOR" "No running containers to stop.")"
    exit 0
fi
...
echo
read -p "Select a container to stop [1-${#containers[@]}]: " idx
if [[ $idx =~ ^[0-9]+$ ]] && [ $idx -ge 1 ] && [ $idx -le ${#containers[@]} ]; then
    cname="${containers[$((idx-1))]}"
    docker stop "$cname"
    print_box "$(print_colored "$SUCCESS_COLOR" "Stopped container: $cname")"
else
