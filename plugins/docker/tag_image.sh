...
if [ ${#images[@]} -eq 0 ]; then
    print_box "$(print_colored "$WARNING_COLOR" "No images to tag.")"
    exit 0
fi
...
echo
read -p "Select an image to tag [1-${#images[@]}]: " idx
if [[ $idx =~ ^[0-9]+$ ]] && [ $idx -ge 1 ] && [ $idx -le ${#images[@]} ]; then
    ...
    if [ -n "$new_repo" ] && [ -n "$new_tag" ]; then
        if docker tag "$repo:$tag" "$new_repo:$new_tag"; then
            print_box "$(print_colored "$SUCCESS_COLOR" "Successfully tagged $repo:$tag as $new_repo:$new_tag")"
        else
            print_box "$(print_colored "$ERROR_COLOR" "Failed to tag image")"
        fi
    else
        print_box "$(print_colored "$ERROR_COLOR" "Repository name and tag are required.")"
    fi
else
