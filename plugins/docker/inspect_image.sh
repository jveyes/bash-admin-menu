...
if [ ${#images[@]} -eq 0 ]; then
    print_box "$(print_colored "$WARNING_COLOR" "No images to inspect.")"
    exit 0
fi
...
echo
read -p "Select an image to inspect [1-${#images[@]}]: " idx
if [[ $idx =~ ^[0-9]+$ ]] && [ $idx -ge 1 ] && [ $idx -le ${#images[@]} ]; then
    IFS='|' read -r repo tag id <<< "${images[$((idx-1))]}"
    print_box "$(print_colored "$INFO_COLOR" "Inspecting $repo:$tag ...")"
