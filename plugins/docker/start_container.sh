...
if [ ${#containers[@]} -eq 0 ]; then
    print_box "$(print_colored "$WARNING_COLOR" "No stopped containers to start.")"
    exit 0
fi
...
echo
read -p "Select a container to start [1-${#containers[@]}]: " idx
if [[ $idx =~ ^[0-9]+$ ]] && [ $idx -ge 1 ] && [ $idx -le ${#containers[@]} ]; then
    cname="${containers[$((idx-1))]}"
    docker start "$cname"
    print_box "$(print_colored "$SUCCESS_COLOR" "Started container: $cname")"
else
