...
if [ ${#containers[@]} -eq 0 ]; then
    print_box "$(print_colored "$WARNING_COLOR" "No containers available to view logs.")"
    exit 0
fi
...
echo
read -p "Select a container to view logs [1-${#containers[@]}]: " idx
if [[ $idx =~ ^[0-9]+$ ]] && [ $idx -ge 1 ] && [ $idx -le ${#containers[@]} ]; then
    cname="${containers[$((idx-1))]}"
    print_box "$(print_colored "$INFO_COLOR" "Logs for $cname:")"
    docker logs "$cname"
else
